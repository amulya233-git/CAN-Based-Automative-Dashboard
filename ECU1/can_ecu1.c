...
void init_can(void) {
    ...
    /*
     * Enable Filters
     * Filter 0, filter is not used bcz we have 2 msz if we use filter it will only send 1 msz
     */
    RXFCON0 = 0x00;
    ...
    RXB0CON = 0x00;
    //This is bcz to make the buffer full
    RXB0CONbits.RXM0 = 1;
    RXB0CONbits.RXM1 = 1;
}
...
//we have 8bit must convert to 11bit bcz we use standard id , we fill msb as 0
...
void can_transmit(uint16_t msg_id, const uint8_t *data, uint8_t len) {
     ...
     * */
    //directly storing data to address
    ptr = (uint8_t *) & TXB0D0;
    for (int i = 0; i < len; i++) {
        ptr[i] = data[i];
    }
